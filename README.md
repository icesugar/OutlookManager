...
镜像地址：
`tanghx999/microsoft-email-manager:main`
...
- 当前仓库内的 [docker-compose.yml](./docker-compose.yml) 默认会拉取镜像 `tanghx999/microsoft-email-manager:main`
...
3. 填入镜像：`tanghx999/microsoft-email-manager:main`
4. 挂载持久化卷到 `/app/data`。
5. 设置与上面一致的环境变量。
6. 发布后访问域名完成初始化。
...
- 这个项目最适合直接使用公开镜像 `tanghx999/microsoft-email-manager:main`
- 数据目录仍然应挂载到 `/app/data`
...
3. 创建一个新的应用。
4. 选择容器镜像部署方式。
5. 填入镜像：`tanghx999/microsoft-email-manager:main`
6. 将服务端口设置为 `8000`。
7. 配置环境变量：
   `HOST=0.0.0.0`
   `PORT=8000`
